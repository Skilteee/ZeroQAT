...
def get_act_scales(model, tokenizer, task, num_samples=512, seq_len=512, encode_prompt=None):
    model.eval()
    device = next(model.parameters()).device
    act_scales = {}
    calibration_dataset = task.valid_samples
    ...
    for i in tqdm(range(min(num_samples, len(calibration_dataset)))):
        encoded_candidates, option_lens = encode_prompt(
            task, task.get_template(), [], calibration_dataset[i], tokenizer,
            max_length=seq_len,
            generation=task.generation, max_new_tokens=50
        )
