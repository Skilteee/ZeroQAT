...
class QuantLlamaMLP(nn.Module):
    ...
    def init_smoothing(self, scales, shifts):

        self.gate_proj.init_smoothing([scales], [shifts])
        self.up_proj.init_smoothing([scales], [shifts])

        self.gate_proj.smooth = mlp_smooth
        self.up_proj.smooth = mlp_smooth
        self.down_proj.smooth = mlp_down_proj_smooth
...
class QuantLlamaAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.o_proj = QuantLinear(
            org_module.o_proj, quant_args['weight_quant_params'], quant_args['act_quant_params'], zo_eps=zo_eps
    ...
    #     self.mlp.init_smoothing(self.fc1_smooth_scale, self.fc1_smooth_shift)
    ...
    def forward(
        ...
        # max_size = self.config.max_position_embeddings if
...
class QuantLlamaDecoderLayer(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        quant_args['weight_quant_params']['lwc'] = False
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states, self.idx)
