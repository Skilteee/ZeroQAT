...
from quantize.int_linear import QuantLinear, q_smooth, k_smooth, v_smooth, o_smooth, mlp_smooth, mlp_down_proj_smooth
...
class QuantOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = QuantLinear(
            org_module.k_proj,
            args['weight_quant_params'], # {'n_bits': 8, 'per_channel_axes': [0], 'symmetric': False, 'dynamic_method': 'per_channel', 'group_size': None, 'lwc': True, 'disable_zero_point': False}
            args['act_quant_params'], # {'n_bits': 4, 'per_channel_axes': [], 'symmetric': False, 'dynamic_method': 'per_token'}
            zo_eps=zo_eps,
        )
        ...
        self.out_proj = QuantLinear(
            org_module.out_proj, args['weight_quant_params'], args['act_quant_params'], zo_eps=zo_eps,
        )
        self.qkt_matmul = QuantMatMul(
            args['act_quant_params'], args['act_quant_params'], matmul_func=torch.bmm
        )
        self.pv_matmul = QuantMatMul(
            args['p_quant_params'], args['act_quant_params'], matmul_func=torch.bmm
        )
    ...
    def init_smoothing(self, scales: List[torch.Tensor], shifts: List[torch.Tensor]):

        self.q_proj.init_smoothing([scales[0], scales[2]], [shifts[0]])
        self.k_proj.init_smoothing([scales[0], scales[2]], [shifts[0]])
        self.v_proj.init_smoothing([scales[0], scales[1]], [shifts[0], shifts[1]])

        self.out_proj.init_smoothing([scales[1]], [shifts[1]])

        self.q_proj.smooth = q_smooth
        self.k_proj.smooth = k_smooth
        self.v_proj.smooth = v_smooth
        self.out_proj.smooth = o_smooth
    ...
    def forward(
        ...
        # if attention_mask.device != self.q_proj.temp_weight.device:
        #     hidden_states = hidden_states.to(self.q_proj.temp_weight.device)
        #     attention_mask = attention_mask.to(self.q_proj.temp_weight.device)
...
class QuantOPTDecoderLayer(nn.Module):
    def __init__(
        ...
        quant_args['weight_quant_params']['lwc'] = True
        self.self_attn = QuantOPTAttention(
            ...
            is_decoder=True,
            bias=config.enable_bias, # true
            args=quant_args,
            zo_eps=zo_eps
        )
        self.do_layer_norm_before = config.do_layer_norm_before # true
        ...
        self.fc1 = QuantLinear(
            ori_layer.fc1,
            weight_quant_params=quant_args['weight_quant_params'], # {'n_bits': 8, 'per_channel_axes': [0], 'symmetric': False, 'dynamic_method': 'per_channel', 'group_size': None, 'lwc': False, 'disable_zero_point': False}
            act_quant_params=quant_args['act_quant_params'], # {'n_bits': 4, 'per_channel_axes': [], 'symmetric': False, 'dynamic_method': 'per_token'}
            zo_eps=zo_eps,
        )
    ...
    def init_smoothing(self):
        ...
        self.self_attn.init_smoothing([self.qkv_smooth_scale, self.out_smooth_scale, self.qkt_smooth_scale],
                                      [self.qkv_smooth_shift, self.out_smooth_shift])
        self.fc1.init_smoothing([self.fc1_smooth_scale], [self.fc1_smooth_shift])


        self.fc1.smooth = mlp_smooth
        self.fc2.smooth = mlp_down_proj_smooth
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.self_attn.q_proj.use_temporary_parameter:
        ...
        if self.do_layer_norm_before:
            # if hidden_states.device != self.self_attn_layer_norm.temp_weight.device:
            #     hidden_states = hidden_states.to(self.self_attn_layer_norm.temp_weight.device)
            #     residual = residual.to(self.self_attn_layer_norm.temp_weight.device)
            #     attention_mask = attention_mask.to(self.self_attn_layer_norm.temp_weight.device)
