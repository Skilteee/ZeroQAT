...
class W8A8Linear(nn.Module):
    def __init__(
        ...
        if act_quant == "per_token":
            ...
            self.act_quant = partial(quantize_activation_per_token_absmax, n_bits=n_bits)
        # elif act_quant == "per_tensor":
        #     self.act_quant_name = "per_tensor"
        #     self.act_quant = partial(quantize_activation_per_tensor_absmax, n_bits=n_bits)
        else:
    ...
    @staticmethod
    def from_float(
        module, weight_quant="per_channel", act_quant="per_token", quantize_output=False, name=None, n_bits=8
    ):
    ...
    for name, m in model.model.named_modules():
        if isinstance(m, OPTDecoderLayer):
            m.fc1 = W8A8Linear.from_float(
                m.fc1, weight_quant=weight_quant, act_quant=act_quant, name='fc1_{}'.format(idx), n_bits=n_bits
            )
            m.fc2 = W8A8Linear.from_float(
                m.fc2, weight_quant=weight_quant, act_quant=act_quant, name='fc2_{}'.format(idx), n_bits=n_bits
            )
            idx += 1
            ...
            m.v_proj = W8A8Linear.from_float(
                ...
                act_quant=act_quant,
                quantize_output=quantize_bmm_input,
                name='v_proj_{}'.format(idx-1),
                n_bits=n_bits
            )
            m.out_proj = W8A8Linear.from_float(
                m.out_proj, weight_quant=weight_quant, act_quant=act_quant, name='out_proj_{}'.format(idx-1), n_bits = n_bits
            )
    return model
